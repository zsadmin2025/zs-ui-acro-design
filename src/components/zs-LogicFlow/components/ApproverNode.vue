  ...
  <div class="node-wrap-box">
    <div>
      <div class="title" :style="{ background: '#FF943E' }">
        ...
        <i class="anticon anticon-close close" @click="handleDelete"></i>
      </div>
      <div class="content">
...
<script setup lang="ts">
  ...
  const title = computed(() => props.node.properties?.title || '审核人');
  const assignee = computed(
    () => props.node.properties?.assignee || '直接主管',
  );

  const handleDelete = () => {
    lf.deleteNode(props.node.id);
  };
