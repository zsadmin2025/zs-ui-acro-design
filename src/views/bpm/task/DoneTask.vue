    ...
    <zs-container layout="header-main-footer">
      ...
      <template #main-header>
        <a-row justify="space-between" align="center">
          <a-col :xs="24" :sm="12">
            <span class="page-subtitle">已办任务</span>
          </a-col>
          <a-col
      ...
      <template #main-body>
        ...
        >
          <template #result="{ record }">
            ...
            <a-tag v-else-if="record.result === 'REJECT'" color="red"
              >驳回</a-tag
            >
            <a-tag v-else-if="record.result === 'TRANSFER'" color="blue"
              >转办</a-tag
            >
            <a-tag v-else-if="record.result" color="arcoblue">{{
              record.result
            }}</a-tag>
            <span v-else>-</span>
          </template>
...
<script lang="ts" setup>
  ...
  const list = ref<(TaskItem & { result?: string })[]>([]);
  const total = ref(0);
