      ...
      <div class="info-card">
        <div class="info-card__title-row">
          ...
          <span
            :class="[
              'status-tag',
              `status-tag--${getStatusColor(detail?.processInstance?.status)}`,
            ]"
          >
          ...
          <a-tabs v-model:active-key="activeTab" class="detail-tabs">
            ...
            <a-tab-pane key="diagram" title="流程图">
              <div class="diagram-box">
                <zs-workflow-viewer v-if="modelJson" :model-json="modelJson" />
  ...
  const loadDetail = async () => {
    ...
    try {
      ...
      const mapNodeResult = (status?: string): string => {
        ...
        return 'PENDING'; // RUNNING 及其他均视为进行中
      };
