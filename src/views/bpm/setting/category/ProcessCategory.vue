    ...
    >
      <a-form ref="formRef" :model="store.form" auto-label-width>
        <a-form-item
          ...
          ><a-input v-model="store.form.name" placeholder="请输入分类名称"
        /></a-form-item>
        <a-form-item label="编码" field="code"
          ><a-input v-model="store.form.code" placeholder="请输入编码"
        /></a-form-item>
        <a-form-item label="排序" field="sort"
          ><a-input-number v-model="store.form.sort" :min="0" mode="button"
        /></a-form-item>
...
<script lang="ts" setup>
  ...
  import { useProcessCategoryStore } from '@/store/modules/bpm/setting/processCategoryStore';
  import { useAppStore } from '@/store';
  import DensityDropdown from '@/components/density-dropdown/index.vue';

  const store = useProcessCategoryStore();
