    ...
    <zs-container layout="header-main-footer">
      ...
      <template #main-header>
        <a-row justify="space-between" align="center">
          <a-col :xs="24" :sm="12"
            ><span class="page-subtitle">流程实例管理</span></a-col
          >
          <a-col
        ...
        >
          <template #status="{ record }">
            <a-tag v-if="record.status === 'RUNNING'" color="blue"
              >运行中</a-tag
            >
            <a-tag v-else-if="record.status === 'SUSPENDED'" color="orange"
              >已挂起</a-tag
            >
          ...
          <template #operations="{ record }">
            <a-space size="mini">
              <a-link
                ...
                ><template #icon><icon-pause-circle /></template>挂起</a-link
              >
              <a-link
...
<script lang="ts" setup>
  ...
  import { useProcessInstanceStore } from '@/store/modules/bpm/setting/processInstanceStore';
