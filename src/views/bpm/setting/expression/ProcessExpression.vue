<template>
  <div class="process-expression">
    ...
    <a-modal
      ...
      :title="store.form.id ? '编辑表达式' : '新建表达式'"
    ...
    >
      <a-form ref="formRef" :model="store.form" auto-label-width>
        <a-form-item
          ...
          ><a-input v-model="store.form.name" placeholder="请输入表达式名称"
        /></a-form-item>
        <a-form-item
          label="表达式"
          field="expression"
          ...
          ><a-textarea
            v-model="store.form.expression"
            placeholder="请输入表达式，如：${execution.setVariable('key', value)}"
...
<script lang="ts" setup>
  ...
  import { useProcessExpressionStore } from '@/store/modules/bpm/setting/processExpressionStore';
  import { useAppStore } from '@/store';
  import DensityDropdown from '@/components/density-dropdown/index.vue';

  const store = useProcessExpressionStore();
