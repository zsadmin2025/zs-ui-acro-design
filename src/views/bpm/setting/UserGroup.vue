    ...
    >
      <a-form ref="formRef" :model="form" auto-label-width>
        ...
        <a-form-item label="分组编码" field="groupCode">
          <a-input v-model="form.groupCode" placeholder="请输入分组编码" />
        </a-form-item>
...
<script lang="ts" setup>
  ...
  const handleEdit = async (record: UserGroupItem) => {
  ...
  const handleUserSelect = (val: any) => {
    if (val) {
      ...
      form.userIds = users.map((u: any) => u.userId);
      form.userNames = users.map((u: any) => u.realName);
    }
  };
