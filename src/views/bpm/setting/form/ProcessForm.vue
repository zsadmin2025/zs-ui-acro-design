          ...
          <template #operations="{ record }">
            <a-space size="mini">
              ...
              <a-link
                ...
                status="success"
                @click="store.handlePublish(record)"
                ><template #icon><icon-send /></template>发布</a-link
              >
...
<script lang="ts" setup>
  ...
  import { useProcessFormStore } from '@/store/modules/bpm/setting/processFormStore';
  import { useAppStore } from '@/store';
  import DensityDropdown from '@/components/density-dropdown/index.vue';

  const router = useRouter();
  const store = useProcessFormStore();
  ...
  const handleAdd = () => {
    router.push({ path: '/bpm/setting/form/edit' });
  };

  const handleEdit = (record: any) => {
    router.push({ path: '/bpm/setting/form/edit', query: { id: record.id } });
  };
